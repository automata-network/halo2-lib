...
pub fn ec_double<F: PrimeField, FC: FieldChip<F>, C: CurveAffine<Base = FC::FieldType>>(
    chip: &FC,
    ctx: &mut Context<F>,
    P: &EcPoint<F, FC::FieldPoint>,
) -> EcPoint<F, FC::FieldPoint> {
    ...
    C: CurveAffine<Base = FC::FieldType>,
{
    ...
    rhs = chip.add_constant_no_carry(ctx, &rhs, FC::fe_to_constant(C::a()));
...
impl<F: PrimeField, FC: FieldChip<F>> EccChip<F, FC> {
    ...
    pub fn double<C: CurveAffine<Base = FC::FieldType>>(
        &self,
        ctx: &mut Context<F>,
        P: &EcPoint<F, FC::FieldPoint>,
    ) -> EcPoint<F, FC::FieldPoint> {
    ...
    FC: Selectable<F, Point = FC::FieldPoint>,
{
    ...
    pub fn scalar_mult<C: CurveAffine<Base = FC::FieldType>>(
        &self,
        ctx: &mut Context<F>,
        P: &EcPoint<F, FC::FieldPoint>,
        scalar: &Vec<AssignedValue<F>>,
